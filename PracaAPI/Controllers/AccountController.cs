...
using PracaAPI.Providers;
using PracaAPI.Results;
    ...
    public class AccountController : ApiController
    {
        ...
        public async Task<IHttpActionResult> GetExternalLogin(string provider, string error = null)
        {
            ...
            if (hasRegistered)
            {
                ...
                string role = _userManager.GetRoles(user.Id).FirstOrDefault();
                AuthenticationProperties properties = ApplicationOAuthProvider.CreateProperties(user.UserName, Newtonsoft.Json.JsonConvert.SerializeObject(role));
        ...
        public async Task<IHttpActionResult> Register([FromBody] RegisterViewModel model)
        {
            if (!ModelState.IsValid)
            {
                var errors = this.ModelState.Values.SelectMany(m => m.Errors);/*ModelState.Select(x => x.Value.Errors)
                           .Where(y => y.Count > 0)
                           .ToList();*/
            ...
            /* adding 'User' Role */
            var adding_role = await UserManager.AddToRoleAsync(user.Id, "User");
